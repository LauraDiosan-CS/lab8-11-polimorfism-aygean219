...
Magazin::Magazin(string cod, string nume, double pret, string data_introducerii, double  nr_exemplare_disponibile)
{
	this->nume = nume;
...
void Magazin::setCod(string cod) {
	this->cod = cod;
}
...
{
	return cod + delim + nume + delim + convertDoubleToString(pret) + delim + data_introducerii + delim + convertDoubleToString(nr_exemplare_disponibile);
}